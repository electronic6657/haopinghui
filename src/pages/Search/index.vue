    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="details clearfix">
          ...
          <!-- page-size:当前页条数  current-page：当前页数  current-change：切换页面-->
          <!-- <el-pagination
...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      //但是你把相应的字段变为undefined，当前这个字段不会带给服务器(性能优化)
  ...
  computed: {
    //mapGetters里面的写法：传递的数组，因为getters没有划分模块
    //如果开了命名空间namespaced：true 可以下面这样写
  ...
  watch: {
    //监听路由的信息是否发生变化，如果发生变化就重新发起请求
    $route(bewValue, oldValue) {
      //再次发起请求前整理发给服务器的参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      //再次发起ajax请求
      this.getData();
      //分类名字与关键字不用清理：每一次请求完毕，应该把相应的id1,2,3清空，让他接受下一次的id
      (this.searchParams.category1Id = undefined),
        (this.searchParams.category2Id = undefined),
        (this.searchParams.category3Id = undefined);
    }
