          ...
          <div class="choose">
            ...
            >
              ...
              <dl>
                ...
                <!--  遍历spuSaleAttrValueList数组 ，动态绑定高亮-->
                <dd
                  changepirce="0"
                  :class="{active:value.isChecked==1}"
                  v-for="(value,index) in spuSaleAttr.spuSaleAttrValueList"
                  :key="value.id"
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" class="itxt"  v-model="skuNum" @blur="changeSkuNum"/>
                <button href="javascript:" class="plus" @click="skuNum++">+</button>
                <!-- 最小不小于1 -->
                <button href="javascript:" class="mins" style="margin-top:2px;" @click="skuNum>1?skuNum--:skuNum=1">-</button>
              </div>
              <div class="add">
...
export default {
  ...
  methods: {
    //派发action获取detail数据，要携带skuId（存在路由信息中）
    getData() {
    ...
    addShopCart(){
      ...
      //3、失败，给用户进行提升
      //因为dispatch只能携带两个参数，第二个参数以对象的形式发送
      this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuId,skuNum:this.skuNum}).then(res =>{
        //在进行路由跳转的时候还需要将产品的信息带给下一个组件
        //一些简单的数据可以skuNum可以通过query传递
        //比较复杂的数据，通过回话存储（不持久化，会话结束后数据消失）
        //本地存储|会话存储，一般存储的是字符串对象
